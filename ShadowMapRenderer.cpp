...
void ShadowMapRenderer::prepare(vec3 lightDirection)
{
	...
	mat4 mvpMatrix = projectionViewMatrix * mat4(1.0);
...
void ShadowMapRenderer::update_light_view_matrix(vec3 direction, vec3 center)
{
	...
	center = -center;
	lightViewMatrix = mat4(); // Create identity matrix
	float pitch = acos(vec2(direction.x, direction.z).length());
...
{
	projectionMatrix = mat4(); // Set to identity matrix
...
{
	shadowBox.update();
	vec3 sunPosition = vec3(100000, 100000, -150000);
