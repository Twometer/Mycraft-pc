...
void VboBuilder::drawDisplacedVertices(const GLfloat* textures, const GLfloat* vertices, int x, int y, int z, int texX, int texY, GLfloat col) {
...
unsigned char VboBuilder::getBlock(unsigned char me, int x, int y, int z) {
	unsigned char id = 0;
	if (x < 0 || y < 0 || z < 0 || x > 15 || y > 15 || z > 15) id = OpenGLRenderer::world->getBlock(xo + x, yo + y, zo + z);
	else id = chunk->getBlock(x, y, z);
	if (!BlockRegistry::isTransparent(me) && BlockRegistry::isTransparent(id))
		return 0;
	else return id;
}